...
def get_line_segment(ins: Instruction, pos: Position) -> (LineSegment, Position):
	move = move_dict[ins.direction]
	next_pos = move(pos.x, pos.y, ins.distance)

	return LineSegment(pos, next_pos, ins.distance), next_pos

def generate_wire(instructions: List[Instruction]) -> List[LineSegment]:
	pos = Position(0,0)
	wire: List[LineSegment] = []
	for ins in instructions:
		line_seg, pos = get_line_segment(ins, pos)
		wire.append(line_seg)
	return wire

def between(a: int, b: int, x: int) -> bool:
	return a < x < b or a > x > b

def find_intersections(wire_a: List[LineSegment], wire_b: List[LineSegment]) -> List[Position]:
	intersections: List[(Position, int)] = []
	...
	for seg_a in wire_a:
		...
		for seg_b in wire_b:
			if between(seg_a.start.x, seg_a.end.x, seg_b.start.x) and between(seg_b.start.y, seg_b.end.y, seg_a.start.y):
				...
				intersections.append(Position(seg_b.start.x, seg_a.start.y, total_steps_taken))
			elif between(seg_b.start.x, seg_b.end.x, seg_a.start.x) and between(seg_a.start.y, seg_a.end.y, seg_b.start.y):
				total_steps_taken = wire_a_steps_taken+wire_b_steps_taken + abs(seg_b.start.x - seg_a.start.x) + abs(seg_b.start.y - seg_a.start.y)
				intersections.append(Position(seg_a.start.x, seg_b.start.y, total_steps_taken))
...
if __name__ == '__main__':
	...
	instructions_a = [get_instruction(raw_ins) for raw_ins in raw_wire_a]
	instructions_b = [get_instruction(raw_ins) for raw_ins in raw_wire_b]
	
	wire_a = generate_wire(instructions_a)
	wire_b = generate_wire(instructions_b)

	intersections = find_intersections(wire_a, wire_b)
	min_distance = min(map(lambda pos: abs(pos.x)+abs(pos.y), intersections))
	print(f'Part 1: {min_distance}')
	min_steps = min(map(lambda pos: pos.distance, intersections))
